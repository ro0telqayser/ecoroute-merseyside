import { WEEK, LEADERBOARD, ACHIEVEMENTS } from "../data";
import { IconFlame, IconStar, IconLock } from "../icons";

export function StatsScreen({ totalSaved, totalPoints, totalTrips }) {
  const maxWeek = Math.max(...WEEK.map((w) => w.v), 0.1);
  const weekSum = WEEK.reduce((a, b) => a + b.v, 0);
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
            <span className="tag" style={{ background: "rgba(255,255,255,0.12)", color: "#fff" }}>
              <IconFlame size={12} /> 13 day streak
            </span>
            <span className="tag" style={{ background: "rgba(255,255,255,0.12)", color: "#fff" }}>
              <IconStar size={12} /> Level 4 · Commuter
            </span>
          </div>
        </div>
          ...
          {ACHIEVEMENTS.map((a, i) => (
            <div key={i} className={`achievement ${a.earned ? "earned" : "locked"}`}>
              <div className="ach-ico">
                {a.earned ? <a.Ico size={16} /> : <IconLock size={14} />}
              </div>
